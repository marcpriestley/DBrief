...
- **Squad (Accountability Pairs + Leaderboard + Challenges)**: Users can connect with trusted people via username search. Accepted connections see each other's streak, 7-day consistency %, today's avg score, 30-day avg, and last logged date. Journal, goal content, debriefs stay private. Connection requests trigger push notifications. Found at `/squad` via the nav tab. The "Board" tab shows a ranked leaderboard of yourself + all accepted connections, sortable by streak, 7-day consistency, or score. Ties broken by secondary metric. Rank badges for top 3 (crown, silver, bronze). The "Challenges" tab supports habit challenges (shared daily habit completion) and score challenges (everyone tracks the same metric, highest average wins). Creator can set invite-only (pick specific connections) or open (all connections auto-invited). Challenge leaderboards show live standings per challenge. Tables: challenges, challenge_participants, challenge_logs.
