...
- **Daily Goals**: Task list of recurring daily actions (shown as "Today's Goals" in the UI) — differentiated from habits. Starts blank for new users; they add their own.
- **Habit Lab**: Separate habit formation feature. Users build new behavioral routines (distinct from daily task goals). Supports per-habit streaks, 66-day milestone progress, habit stacking (anchor to existing routine via implementation intentions: "After I X, I will Y"), AI-guided 4-step setup (name/emoji/multi-select categories → motivation → habit stack → reminder time), per-habit customizable push notification reminders. AI debrief is aware of active habits and their completion status for motivational nudges. "Make my bed" (🛏️, anchored to "I wake up") is seeded as the first habit for every new user and for existing users on first Habit Lab visit.
